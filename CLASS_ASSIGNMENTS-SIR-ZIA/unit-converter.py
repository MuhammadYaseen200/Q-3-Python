...
temp_units = ["Celsius", "Farenheit", "Kelvin"]


category = st.selectbox("Categories", list(units.keys()) + ["Temperature"])
...
str_value = st.text_input("Enter value to convert", "")
    ...
    try:
        ...
        if category == "Temperature":
            ...
            if from_unit == to_unit:
                ...
                base_val = value * unit_dict[from_unit]
                result = base_val / unit_dict[to_unit]
                formula = f"{value} x {unit_dict[from_unit]} ÷ {unit_dict[to_unit]} = {result}"
